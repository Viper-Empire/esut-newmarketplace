...
- [ ] Diagnose and fix all marketplace flows that receive an HTML document where a JSON/tRPC response is expected (`Unexpected token '<'`). (Global mitigations are in place, but close this item only after every marketplace data-fetching entry point is confirmed to use the guarded transport and the original failure mode is reproduced safely.)
