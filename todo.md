...
- [ ] Diagnose and fix all marketplace flows that receive an HTML document where a JSON/tRPC response is expected (`Unexpected token '<'`). (The original console error coincided with a Vite connection loss during a development restart. Unknown `/api/*` routes now return structured JSON before SPA fallback and the shared tRPC transport rejects non-JSON responses safely; additional affected-flow reproduction remains tracked.)
