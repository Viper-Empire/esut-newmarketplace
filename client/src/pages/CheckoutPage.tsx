...
export default function CheckoutPage() {
  ...
  if (!cart.data?.items.length) return <main className="page-shell py-20 text-center"><h1 className="text-3xl font-extrabold">Your cart is empty</h1><p className="mt-2 text-slate-600">Return to the catalogue to find a real listing before starting checkout.</p><Link href="/explore"><Button className="mt-6 bg-[#e31b23]">Continue shopping</Button></Link></main>;

  return <main className="page-shell py-8">
    <div className="flex flex-wrap items-center justify-between gap-3"><Link href="/cart" className="inline-flex items-center text-sm font-bold text-[#00843d] underline-offset-4 hover:underline">← Back to cart</Link><span className="text-xs font-bold uppercase tracking-[.12em] text-slate-400">Protected buyer checkout</span></div>
    <p className="eyebrow mt-5 text-[#00843d]">SECURE CHECKOUT</p>
    ...
    <div className="mt-7 grid gap-7 lg:grid-cols-[1fr_350px]">
      <section className="space-y-5">
        ...
        <div className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200"><div className="flex items-center justify-between gap-3"><label htmlFor="pickup-note" className="font-bold">Pickup note <span className="font-normal text-slate-400">(optional)</span></label><span className="text-xs font-semibold text-slate-400">{note.length}/500</span></div><textarea id="pickup-note" maxLength={500} value={note} onChange={event => setNote(event.target.value)} className="mt-3 min-h-28 w-full rounded-xl border p-3 outline-none focus:border-[#00843d]" placeholder="For example: I will be around the Faculty of Engineering at 2pm." /></div>
      </section>
      <aside className="h-fit rounded-2xl bg-[#111827] p-6 text-white"><h2 className="font-bold">Your order</h2>{cart.data.items.map(row => <div className="mt-4 flex justify-between gap-4 text-sm" key={row.item.id}><span className="text-slate-300">{row.listing.title} × {row.item.quantity}</span><b>{naira(row.listing.priceKobo * row.item.quantity)}</b></div>)}<div className="my-5 border-t border-white/15" /><div className="flex justify-between text-lg font-extrabold"><span>Total</span><span>{naira(cart.data.totalKobo)}</span></div><Button disabled={place.isPending} aria-busy={place.isPending} onClick={() => place.mutate({ pickupNote: note.trim() || undefined, idempotencyKey })} className="mt-6 w-full bg-[#e31b23] hover:bg-[#c9161d]">{place.isPending ? "Placing order…" : "Place order"} <CheckCircle2 size={17} /></Button><p className="mt-4 text-center text-xs text-slate-400">The server independently recalculates all item prices and reserves stock before creating an order.</p></aside>
    </div>
  </main>;
}
