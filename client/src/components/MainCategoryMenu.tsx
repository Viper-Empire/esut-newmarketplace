...
export function MainCategoryMenu({ categories, inline = false }: { categories: CategoryNode[]; inline?: boolean }) {
  ...
  useEffect(() => {
    ...
    const closeOnPageScroll = () => close();
    const closeOnOutsideTouch = (event: PointerEvent) => {
      const target = event.target;
      if (target instanceof Node && !menuRootRef.current?.contains(target)) close();
    };
  ...
  return <div ref={menuRootRef} className={inline ? "w-full" : "relative"}>
